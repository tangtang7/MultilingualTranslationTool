...
class Excel2XmlUtils:
    ...
    def convert(self, sheet):
        ...
        try:
            ...
            Log.error(Constant.Error(Constant.EXCEPTION_EXL_FILE, e.message).get_desc())
            return Constant.Error(Constant.EXCEPTION_EXL_FILE, e.message)

        if len(firstRow) == 0:
            Log.error(Constant.Error(Constant.ERROR_KEY_NOT_FOUND).get_desc())
        ...
        if self.filePath and tempLanguageIndex:  # 输入是文件，指定目标语言
            ...
            del xlsValues[0]
            Log.info(f"xlsValues = {xlsValues}")
        ...
        if not self.dirPath:  # 目录为空，返回
            Log.error(Constant.Error(Constant.ERROR_IMPORT_INPUT).get_desc())
            return Constant.Error(Constant.ERROR_IMPORT_INPUT)

        if not os.path.exists(self.dirPath):
            Log.error(Constant.Error(Constant.ERROR_DIR_NOT_EXIST).get_desc())
...
def main():
    ...
    options = addParser()
    excel2XmlUtils.xls2xml_options(options)
